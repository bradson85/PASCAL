$(document).ready(function () {
    ...
    $('#addRow').click(function (event) {
        ...
        var rows = $('<tr><td>0</td>' +
            '<td contenteditable= "true">Enter A New Categorie</td>' // term name
            +
            '<td contenteditable= "true">Enter New Category Level</td>' /// for level
            +
    ...
    function saveToDB() {
        ...
        $('#word_table tr').each(function (row, tr) {
            ...
            if (count > 0 && ($.trim($(tr).find('td:eq(1)').text()) == "" || $.trim($(tr).find('td:eq(2)').text()) == "")) {
                save = false; //false because of blank field somewhere

            } else {
                TableData[row] = {
                    "ID": $.trim($(tr).find('td:eq(0)').text()),
                    "catName": $.trim($(tr).find('td:eq(1)').text()),
                    "level": $.trim($(tr).find('td:eq(2)').text())   // trim this data to eliminate trailing spaces
                }
            }
